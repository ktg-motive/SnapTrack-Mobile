...
// Helper function to format date for comparison using local time
...
export default function QuickStats({ stats, isLoading, receipts = [] }: QuickStatsProps) {
  ...
  const calculateStats = (timeframeIndex: number) => {
    ...
    case 0: // Today - Fix timezone handling
      ...
      const todayStr = formatDateForComparison(now); // Use local time, not UTC
    ...
    case 1: // This Week
      ...
      const startWeekStr = formatDateForComparison(startOfWeek);
      const endWeekStr = formatDateForComparison(endOfWeek);
...
const styles = StyleSheet.create({
  ...
  cueContainer: {
    ...
    position: 'absolute',
    right: 0,
    top: 0 // Don't interfere with main touch area
  },
