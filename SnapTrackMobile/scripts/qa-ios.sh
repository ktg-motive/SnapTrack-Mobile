...
main() {
    ...
    # 1. Run quality gates (QA mode - allows non-critical errors)
    print_step "Step 1: Quality Gates (QA Mode)"
    if ! ./scripts/pre-release-checks.sh --qa; then
        print_error "Quality gates failed. Fix critical issues before proceeding."
    fi
    ...
    if expo run:ios; then
        ...
        print_warning "expo run:ios failed (likely Node.js ESM issue)"
        echo ""
        echo "Alternative approaches for QA testing:"
        echo "1. Use EAS Build for device testing:"
        echo "   eas build --platform ios --profile preview"
        echo ""
        echo "2. Manual Xcode build:"
        echo "   cd ios && xcodebuild -workspace SnapTrack.xcworkspace -scheme SnapTrack -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 15' build"
        echo ""
        echo "3. Use Expo Go for development:"
        echo "   expo start --dev-client"
        echo ""
        print_error "iOS build failed - see alternatives above"
    fi
}
